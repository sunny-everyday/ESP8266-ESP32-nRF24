...
void loop()
{
	...
	for(int h=0;h<1;h++)
	{
		uint8_t Tempdata[14] ={1,0,0,400,0,0,2,2,2,2,2,2,2,0} ;//buf[13]=2是单色灯命令
		sendDatato2G4(Tempdata);
	}
	delay(410);
	for(int h=0;h<1;h++)
	{
		uint8_t Tempdata[14] ={0,1,0,400,0,0,2,2,2,2,2,2,2,0} ;//buf[13]=2是单色灯命令
		sendDatato2G4(Tempdata);
	}
	delay(410);
	for(int h=0;h<1;h++)
	{
		uint8_t Tempdata[14] ={0,0,1,400,0,0,2,2,2,2,2,2,2,0} ;//buf[13]=2是单色灯命令
		sendDatato2G4(Tempdata);
	}
	delay(410);
